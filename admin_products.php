...
if (isset($_GET['delete'])) {
   ...
   $delete_wishlist = $conn->prepare("DELETE FROM `bidding` WHERE pid = ?");
   ...
   <?php include 'admin_header.php'; ?>
      ...
      <section class="add-products">
         ...
         <form action="" method="POST" enctype="multipart/form-data">
            <div class="flex">
               <div class="inputBox">
                  <input type="text" name="symbol" class="box" required placeholder="Enter Stock Symbol">
               </div>
               <div class="inputBox">
                  <input type="number_format" min="0" name="price" class="box" required placeholder="Enter Stock Price">
                  <input type="number" name="security" class="box" required placeholder="Enter Stock Security">
                  <input type="number" name="profit" class="box" required placeholder="Enter Stock Profit">

                  <div class="form-group row">
                     <div class="col-md-4">
                        <input type="datetime-local" class="box" name="bid_end_datetime" required placeholder="Bidding End Date/Time" value="<?php echo isset($bid_end_datetime) && strtotime($bid_end_datetime) > 0 ? date("Y-m-d H:i", strtotime($bid_end_datetime)) : '' ?>">
                     </div>
                  </div>
            ...
            if ($show_products->rowCount() > 0) {
               while ($fetch_products = $show_products->fetch(PDO::FETCH_ASSOC)) {
                  ...
                  if ($fetch_products['bid_value'] == "await") {
                     # code...
                     echo "<tr><td>" . $fetch_products['id'] . "<td/<td>" . $fetch_products['symbol'] . "<td/<td>" . $fetch_products['price'] . "<td/<td>" . $fetch_products['security'] . "<td/<td>" . $fetch_products['profit'] . "<td/<td>" . $fetch_products['bid_end_datetime'] .
                     "<td/<td>" . "<a class= option href=admin_update_product.php?update=" . $fetch_products['id'] . ">Update</a>" .
                     "<a class= delete onclick=return confirm('delete this product?') href=admin_products.php?delete=" . $fetch_products['id'] . ">Delete</a>" . "</td/<td>";
                  }
               }
               echo "</table>";
            ?>
